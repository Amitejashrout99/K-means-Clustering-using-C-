...
int main () 
{
  ...
  string line;
  int user_input[1][1];
  ifstream myfile ("weather_database1.csv");// used for accessing the dataset that is used for extracting the data upon which the k-means algorithm works upon
  ...
  if (myfile.is_open())
  {
	...
	int no_of_clusters,select;
  	...
  	cin>>select;
  	for(int i=1;i<no_of_clusters+1;i++)
  	{
  	...
  	cluster1[0][0]=cluster_dataset[1][0];// used for shifting the first and the second data of the entire database to the first and second cluster respectively 
	cluster1[0][1]=cluster_dataset[1][1];
	cluster2[0][0]=cluster_dataset[2][0];
	cluster2[0][1]=cluster_dataset[2][1];  	
